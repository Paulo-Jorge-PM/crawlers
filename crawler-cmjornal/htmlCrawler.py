...
class htmlCrawler:
    ...
    def __init__(self, url=None):
        ...
        self.BASE_DOMAINS = ["cmjornal.pt"]#for checking if it is not an external link, can be more than one
        self.SAVE_DIR = "output/html/cmjornal/"
        ...
        if render == True:
           ...
           () => {
                  $(document).ready(function() {                        
                       $(".bt_verComentarios").click();
                  })
            }
            '''
            resp.html.render(sleep=5, script=js, reload=True)#wait=2, sleep=4, script=js, reload=True 
        return resp
